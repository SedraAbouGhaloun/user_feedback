...
def main():
    ...
    n_time_steps = 100  # Total of 100 time steps from start to end

    # Generate linearly spaced time steps between start and end times
    ts = np.linspace(start_time, end_time, n_time_steps) 

    y_from = np.array([0, 0, 0])  # Starting position in 3D space (x=0, y=0, z=0)
    ...
    y_to = np.array([1, 1, 1])  # Target position in 3D space (x=1, y=1, z=1)
    yd_to = np.array([4, 3, 1])  # Target velocity in 3D space (vx=3, vy=2, vz=1)
